...
export default function Chat() {
    ...
    function handleMessage(ev) {
        ...
        if("online" in messageData) {
          ...
          if(messageData.sender === selectedUserIdRef.current || messageData.recipient === selectedUserIdRef.current) {
                                ...
                                {messagesWithoutDupes.map(message => (
                                    <div key={message._id} className={(message.sender === id ? "text-right" : "text-left")}>
                                        <div className={"text-left inline-block p-2 my-2 rounded-md text-sm " + (message.sender === id ? "bg-blue-500 text-white" : "bg-white dark:bg-gray-700 text-gray-500 dark:text-gray-200")}>
                                            ...
                                            {message.file && (
                                                <div className="">
                                                    <a target="_blank" rel="noreferrer" className="flex items-center gap-1 border-b" href={`${axios.defaults.baseURL}/api/uploads/${message.file}`}>
